...
def build_body(contents, tinfo):
    ...
    heads = {
        k: contents.pop(k)
        for k in sorted(contents.keys())
        if k.startswith('Subhead')
    }
    masts = {
        k: contents.pop(k)
        for k in sorted(contents.keys())
        if k.startswith('Masthead')
    }
...
def export_page(page, dest):
    ...
    _ = page.pop("created")
    _ = page.pop("modified")
    _ = page.pop("glass_meta")
    _ = page.pop("last_edit_by")
    _ = page.pop("last_edit_by_email")
    _ = page.pop("last_edit_by_name")
    _ = page.pop("last_published")
    _ = page.pop("last_published_by")
    _ = page.pop("last_published_by_email")
    _ = page.pop("last_published_by_name")
    _ = page.pop("locked_to_groups")
    _ = page.pop("template_id")
    _ = page.pop("site_id")
...
def parse_args():
    ...
    parser.add_argument("--dest", default=pathlib.Path.cwd(),
